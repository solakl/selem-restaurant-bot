...
async def add_to_order(update: Update, category: str, item: str, context):
    ...
    await update.effective_message.reply_text(
        f"✅ Added *{item.replace('_', ' ').title()}* to your cart!\n\nCurrent items: {len(user_orders[uid])}",
        parse_mode='Markdown'
    )
...
async def cancel_upload(update: Update, context):
    ...
    try:
        ...
        pass
    await update.effective_message.reply_text(
        "❌ Upload cancelled.\n\nIf you need to send your receipt later, just click  *Upload Payment Receipt* again.",
        parse_mode='Markdown'
    )
...
async def help_command(u, c):
    await u.effective_message.reply_text(
        ...
        f"/menu - View menu\n"
        f"/order - Place order\n"
        f"/gallery - See gallery\n"
        f"/about - About us\n"
        f"/location - Our location\n"
        f"/help - This message",
        parse_mode='Markdown'
    )

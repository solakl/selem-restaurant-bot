...
# ✅ SECURITY: Get token from environment variable, never hardcode
TOKEN = os.environ.get('TELEGRAM_BOT_TOKEN')

IMAGES_FOLDER = '/home/akl/Desktop/imagebot' # Local path; we'll fix this for Render later
MENU = {
    'traditional': {'name': '🇪 Traditional Ethiopian', 'items': {
    ...
    'fast_food': {'name': ' Fast Food', 'items': {
    ...
    'drinks': {'name': '🥤 Soft Drinks & Beverages', 'items': {
...
RESTAURANT_INFO = {
    'name': 'Selem Restaurant', 'address': 'Bahir Dar, Ethiopia',
    'phone': '+251965895552', 'hours': 'Mon-Sun: 10:00 AM - 11:00 PM',
    ...
    'cbe_account': '1000123456789', 'telebirr_number': '0965895552'
}
...
async def button_handler(update: Update, context):
    ...
    if query.data == 'menu': await show_menu(update)
        ...
        for cat_key in MENU.keys():
            if rest.startswith(cat_key + '_'):
                category, item = cat_key, rest[len(cat_key)+1:]
                break
    ...
    elif query.data.startswith('copy_cbe_'): await query.answer(f"Copied: {query.data.replace('copy_cbe_', '')}", show_alert=True)
    elif query.data.startswith('copy_tele_'): await query.answer(f"Copied: {query.data.replace('copy_tele_', '')}", show_alert=True)
    ...
    elif query.data == 'payment_done': await query.answer("Thank you! Please upload your payment receipt using the button above.", show_alert=True)
...
async def view_cart(update: Update, context):
    ...
    if not valid:
        user_orders[uid] = []
        return await update.effective_message.reply_text("🛒 Your cart is empty or contains invalid items. Please clear and start over.")
...
async def checkout(update: Update, context):
    ...
    kb = [[InlineKeyboardButton("📋 Copy CBE Account", callback_data=f'copy_cbe_{RESTAURANT_INFO["cbe_account"]}'),
...
async def show_gallery(update: Update):
    if not os.path.exists(IMAGES_FOLDER) or not IMAGE_FILES: return await update.effective_message.reply_text("❌ No images available!")
...
def main():
    ...
    if WEBHOOK_URL:
        ...
        print("️ No WEBHOOK_URL found. Running in polling mode for local test only.")
        app.run_polling()
